...
-- Override Config from config (nothing hardcoded; _G.CROW_PlayerConfig or library.config.PlayerSec wins)
do
    local function overlay(dst, src)
        if not src or type(src) ~= "table" then return end
        for k, v in pairs(src) do
            if v ~= nil then
                if k == "characterEffects" and type(v) == "table" and type(dst.characterEffects) == "table" then
                    overlay(dst.characterEffects, v)
                else
                    dst[k] = v
                end
            end
    ...
    if _G.CROW_PlayerConfig and type(_G.CROW_PlayerConfig) == "table" then overlay(Config, _G.CROW_PlayerConfig) end
    local lib = _G.CROW or _G.library
    if lib and lib.config and lib.config.PlayerSec and type(lib.config.PlayerSec) == "table" then overlay(Config, lib.config.PlayerSec) end
end
...
-- Noclip: store original CanCollide per part; when turning OFF, restore only to original (don't set true for parts that were false)
local noclipOriginalCollide = {} -- [part] = original CanCollide (true/false)
...
local function setNoclipPartsCollide(collide)
    ...
    for _, part in pairs(character:GetDescendants()) do
        if part:IsA("BasePart") and part ~= hrp then
            if collide then
                -- Only restore parts we explicitly stored; leave others unchanged (default = current state)
                local orig = noclipOriginalCollide[part]
                if orig ~= nil then
                    part.CanCollide = orig
                end
            else
                -- Before disabling collision, store original if not already stored
