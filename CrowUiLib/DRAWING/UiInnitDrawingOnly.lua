--[[
  UiInnitDrawingOnly.lua - Exact replica of UiInnit.lua using ONLY the Drawing API for UI.
  No Instance.new, ScreenGui, CoreGui, or ImageButton. All visuals and interaction via
  Drawing.new("Square"), Drawing.new("Text"), Drawing.new("Circle"), Drawing.new("Line"),
  Drawing.new("Image"). Requires executor with Drawing API (e.g. KRNL, Fluxus).
]]
local startupArgs = ({...})[1] or {}

local genv = (getgenv and getgenv())
if genv and genv.library and type(genv.library.Unload) == "function" then
...
local next, newInstance, newUDim2, newVector2 = next, (function() return nil end), UDim2.new, Vector2.new
...
local defaultConfig = {
    ...
    SignalUrl = 'https://raw.githubusercontent.com/wrdzy/CROW/main/CrowUiLib/DRAWING/signal.lua';
...
do
    ...
    -- Drawing-only: no Instance.new. Stub so callers don't break (e.g. modal overlay removed).
    function utility:Instance(class, properties)
        return setmetatable({ Parent = nil, Visible = true, Destroy = function() end }, { __index = function() return nil end })
    end
...
function library:init()
    ...
    local s,e = pcall(function()
        ...
        if next(decodedCfg) == nil then
            ...
            -- Spread config apply over multiple frames to avoid detection (no burst of state changes in one frame)
            local entries = {}
            for flag, value in next, decodedCfg do
                entries[#entries + 1] = { flag = flag, value = value }
            end
            local LOAD_CHUNK_SIZE = 10
            local idx = 1
            local function applyChunk()
                local limit = math.min(idx + LOAD_CHUNK_SIZE - 1, #entries)
                for i = idx, limit do
                    local entry = entries[i]
                    local flag, value = entry.flag, entry.value
...
function library:SaveConfig(name)
    ...
    local s,e = pcall(function()
        ...
        -- Save any flags not in options (e.g. toggles from child scripts) so config is complete
        if self.flags then
            for flag, value in next, self.flags do
                if cfg[flag] == nil then
                    if type(value) == "boolean" then
                        cfg[flag] = value and 1 or 0;
                    elseif type(value) == "number" or type(value) == "string" then
                        cfg[flag] = value;
                    elseif type(value) == "table" then
                        cfg[flag] = value;
    ...
    -- Cursor: Triangle if executor supports it, else 3 Lines (Drawing-only; no Instance).
    self.cursor1 = nil
    self.cursor2 = nil
    self.cursor1Lines = nil
    self.cursor2Lines = nil
    ...
    if not okTri then
        local function makeTriangleCursor(color, z)
            local L1 = utility:Draw('Line', {Color = color, Thickness = 2, ZIndex = z}); L1.Visible = false
            local L2 = utility:Draw('Line', {Color = color, Thickness = 2, ZIndex = z}); L2.Visible = false
            local L3 = utility:Draw('Line', {Color = color, Thickness = 2, ZIndex = z}); L3.Visible = false
            return { L1, L2, L3 }
        end
        self.cursor1Lines = makeTriangleCursor(fromrgb(255,255,255), self.zindexOrder.cursor)
        self.cursor2Lines = makeTriangleCursor(fromrgb(85,85,85), self.zindexOrder.cursor-1)
    end
    local function updateCursor()
        local pos = inputservice:GetMouseLocation();
        local vis = self.open
        if self.cursor1 then
            self.cursor1.Visible = vis
            self.cursor2.Visible = vis
            if vis then
                self.cursor1.PointA = pos; self.cursor1.PointB = pos + newVector2(16,5); self.cursor1.PointC = pos + newVector2(5,16)
                self.cursor2.PointA = self.cursor1.PointA; self.cursor2.PointB = self.cursor1.PointB + newVector2(1,1); self.cursor2.PointC = self.cursor1.PointC + newVector2(1,1)
            end
        elseif self.cursor1Lines then
            local A, B, C = pos, pos + newVector2(16,5), pos + newVector2(5,16)
            for _, line in next, self.cursor1Lines do line.Visible = vis end
            if vis then
                self.cursor1Lines[1].From = A; self.cursor1Lines[1].To = B
                self.cursor1Lines[2].From = B; self.cursor1Lines[2].To = C
                self.cursor1Lines[3].From = C; self.cursor1Lines[3].To = A
            end
            local A2, B2, C2 = pos + newVector2(1,1), pos + newVector2(17,6), pos + newVector2(6,17)
            for _, line in next, self.cursor2Lines do line.Visible = vis end
            if vis then
                self.cursor2Lines[1].From = A2; self.cursor2Lines[1].To = B2
                self.cursor2Lines[2].From = B2; self.cursor2Lines[2].To = C2
                self.cursor2Lines[3].From = C2; self.cursor2Lines[3].To = A2
            end
        end
    end

    -- Drawing-only: no ScreenGui/CoreGui. Fullscreen transparent Drawing Square used as modal overlay when menu is open.
    library.screenGui = nil
    library.modalOverlay = nil

    local function ensureModalOverlay()
        if library.modalOverlay and library.drawings[library.modalOverlay.Object] then return library.modalOverlay end
        local cam = workspace.CurrentCamera
        local vp = (cam and cam.ViewportSize) or newVector2(1920, 1080)
        library.modalOverlay = utility:Draw('Square', {
            Size = newVector2(vp.X, vp.Y),
            Position = newVector2(0, 0),
            Transparency = 1,
            Filled = true,
            ZIndex = 0,
        })
        return library.modalOverlay
    end

    utility:Connection(runservice.RenderStepped, function()
        if library.open and library.modalOverlay then
            local cam = workspace.CurrentCamera
            if cam and cam.ViewportSize then
                library.modalOverlay.Size = cam.ViewportSize
                library.modalOverlay.Position = newVector2(0, 0)
            end
        end
    end)

    utility:Connection(library.unloaded, function()
        if library.modalOverlay then pcall(function() library.modalOverlay:Remove() end) library.modalOverlay = nil end
    end)
    ...
    pcall(function()
        if bool then
            ensureModalOverlay()
            if library.modalOverlay then library.modalOverlay.Visible = true end
        elseif library.modalOverlay then
            library.modalOverlay.Visible = false
        end
    end)
            ...
            function tab:AddSection(text, side, order)
                ...
                function section:AddBind(data)
                    ...
                    function bind:SetBind(keybind)
                        ...
                        local keyName = 'NONE'
                        -- Don't accept boolean/nil (e.g. from config or callback state); never index .Name on them
                        if keybind == nil or keybind == true or keybind == false then
                            keybind = 'none'
                        end
                        self.bind = (keybind and keybind ~= 'none') and keybind or self.bind
...
local ok, err = pcall(function()
    ...
    library:init()
    -- When CROW_DRAWING_TEMPLATE_ONLY is true (e.g. loader-drawing.lua), skip full CROW window; template script creates its own.
    if (g.CROW_DRAWING_TEMPLATE_ONLY) then
        g.library = library
        g.CROW = library
        return
    end
