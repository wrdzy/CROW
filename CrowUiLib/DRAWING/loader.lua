--[[
  CrowUiLib DRAWING Loader | Repo: wrdzy/CROW/main/CrowUiLib/DRAWING
  ===================================================================
  Loads signal.lua then UiInnitDrawingOnly.lua (Drawing-only UI library). Returns the library table.
  Requires executor with Drawing API (KRNL, Fluxus, etc.).

  Run: loadstring(game:HttpGet("https://raw.githubusercontent.com/wrdzy/CROW/main/CrowUiLib/DRAWING/loader.lua"))()
...
if not Drawing or type(Drawing.new) ~= "function" then
    error("CROWUI DRAWING Loader: Drawing API missing. Use an executor with Drawing (e.g. KRNL, Fluxus).")
end
...
local baseUrl = (shared.CROW_RAW_URL or "https://raw.githubusercontent.com/wrdzy/CROW/main/CrowUiLib/DRAWING/"):gsub("/$", "") .. "/"
...
local function loadScript(name)
    ...
    if not content or #content == 0 then
        warn("[CROWUI DRAWING Loader] Failed to fetch: " .. url)
        return nil
    end
    local injected = "local _G = (getgenv and getgenv() or _G).CROW_shared or _G\n" .. content
    local fn, err = loadstring(injected)
    if not fn then
        warn("[CROWUI DRAWING Loader] Compile error " .. name .. ": " .. tostring(err))
        return nil
    end
...
-- 2) Drawing-only UI library
local library = loadScript("UiInnitDrawingOnly")
if not library then
    error("CROWUI DRAWING Loader: UiInnitDrawingOnly.lua failed to load.")
end
shared.CROW = library
